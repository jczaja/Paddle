...
std::unique_ptr<PaddlePredictor> InitializePredictor(const std::string& infer_model, std::vector<float>& data, bool use_mkldnn) 
{
  AnalysisConfig cfg;
...
void compare(bool use_mkldnn = false) {
  ...
  auto output = predictor_xx->GetOutputTensor(predictor_xx->GetOutputNames()[0]);
  ...
  // compare results
  auto result = std::equal(xx_output.begin(), xx_output.end(), xx2_output.begin(), [](const float & l, const float & r) { return fabs(l-r) < 1e-4;});

 PADDLE_ENFORCE_EQ(result, true, paddle::platform::errors::Fatal("Results of model run independently differs from results of the same model run as a sequence of models"));
}
