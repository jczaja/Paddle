...
class LRNMKLDNNHandler
    : public platform::MKLDNNHandlerT<T, mkldnn::lrn_forward, mkldnn::lrn_backward> {
 public:
  ...
  LRNMKLDNNHandler(const paddle::framework::ExecutionContext& ctx,
      ...
      : platform::MKLDNNHandlerT<T, mkldnn::lrn_forward, mkldnn::lrn_backward>(
            ...
            platform::CreateKey(dev_ctx, framework::vectorize(in_x->dims()), unique_name)) {
    if (!this->isBwdCached()) {
      ...
      auto src_md =
          mkldnn::memory::desc(dims, platform::MKLDNNGetDataType<T>(), in_x->format());
      auto diff_md =
          mkldnn::memory::desc(dims, platform::MKLDNNGetDataType<T>(), out_grad->format());

      this->AcquireForwardPrimitiveDescriptorNonBlocking( mkldnn::prop_kind::forward_training,
          mkldnn::algorithm::lrn_across_channels, src_md, n, alpha, beta, k);

      this->AcquireBackwardPrimitiveDescriptorNonBlocking(
          mkldnn::algorithm::lrn_across_channels, src_md, diff_md, n, alpha, beta, k);
    }
  }
...
class LRNMKLDNNGradOpKernel : public paddle::framework::OpKernel<T> {
 public:
  void Compute(const paddle::framework::ExecutionContext& ctx) const override {
    ...
    LRNMKLDNNHandler<T> handler(ctx, dev_ctx, ctx.GetPlace(), in_x, out_grad, in_x_grad, ctx.InputName("Out"));
